...
정확한 법령명을 한 줄에 하나씩 출력하세요:"""
    ...
    def _enhance_with_ai(self, text: str, initial_laws: Set[str]) -> Set[str]:
        """AI API를 활용한 법령명 추출 개선 (선택적)"""
        # 이 부분은 사용자가 AI API 키를 설정했을 때만 작동
        # 예시 구현:
        try:
            import openai
            ...
            # ChatGPT API 호출 예시
            prompt = f"""
            다음 텍스트에서 법령명만 정확히 추출해주세요.
            카테고리(상하위법, 행정규칙 등)는 제외하고 실제 법령명만 추출하세요.
            
            텍스트:
            {text[:2000]}  # 토큰 제한을 위해 일부만 전송
            
            출력 형식: 법령명을 한 줄에 하나씩 나열
            """
            
            # AI 응답 처리 (실제 구현 시 API 호출)
            # response = openai.ChatCompletion.create(...)
            # ai_laws = parse_ai_response(response)
            # initial_laws.update(ai_laws)
    ...
    def set_ai_api_key(self, api_key: str, api_type: str = 'openai'):
        """AI API 키 설정"""
        self.use_ai = True
